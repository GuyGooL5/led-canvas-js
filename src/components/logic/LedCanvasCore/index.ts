...
class LedCanvasCore {
    ...
    private _cvs: HTMLCanvasElement = document.createElement('canvas');
    private _ctx: CanvasRenderingContext2D = this._cvs.getContext('2d') as CanvasRenderingContext2D;
    private _template: { active: ImageData; inactive: ImageData; };
    private _board: boolean[][] = [];

    constructor(width: number, height: number, config?: Partial<LedCanvasConfig>) {
